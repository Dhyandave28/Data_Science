...
# Create a list of all unique launch sites for the dropdown options
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Get total successful launches (class 1) for all sites
        total_successes = spacex_df[spacex_df['class'] == 1].groupby('Launch Site')['class'].count().reset_index()
        fig = px.pie(
            total_successes,
            values='class', 
            names='Launch Site', 
        ...
        success_counts['class_label'] = success_counts['class'].apply(
